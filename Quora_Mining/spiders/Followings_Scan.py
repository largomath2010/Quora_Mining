...
class QuestionScanSpider(scrapy.Spider):
    ...
    custom_settings = {
        'ITEM_PIPELINES':{
            'Quora_Mining.pipelines.Save_Following':50,
        },
    }
    ...
    def start_requests(self):
        for item in self.SQLITE['ANSWERS'].select_all():
            ...
            if self.SQLITE['FOLLOWING'].select(list_label=['user_url'],des_label='user_url',source_value=item['answerer_url']):continue
            yield scrapy.Request(url=self.main_domain+item['answerer_url']+'/following',callback=self.parse_followers,
        ...
        for item in self.SQLITE['ASKERS'].select_all():
            ...
            if self.SQLITE['FOLLOWING'].select(list_label=['user_url'], des_label='user_url',source_value=item['asker_url']): continue
            yield scrapy.Request(url=self.main_domain+item['asker_url']+'/following',callback=self.parse_followers,
