...
class QuestionScanSpider(scrapy.Spider):
    ...
    question_add_check=r'.AddQuestionOperationView'
    ...
    asker_css_dict={'asker_name':lambda response: response.css('.feed_item_activity>span>span>a::text').extract_first() if response.css('.feed_item_activity>span>span>a::text').extract_first() else 'Anonymous',
                    'asker_url':lambda response: response.css('.feed_item_activity>span>span>a::attr(href)').extract_first(),
    ...
    def parse_answer(self,response):
        ...
        for log in List_Logs:
            ...
            if not self.answer_css_dict['answer_url'](html_selector):
                check_question_added =html_selector.css(self.question_add_check).extract_first()
